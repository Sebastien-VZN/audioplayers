...
@immutable
class AudioContext {
  AudioContext({AudioContextAndroid? android, AudioContextIOS? iOS}) : android = android ?? const AudioContextAndroid() {
  ...
  bool operator ==(Object other) {
    return identical(this, other) || other is AudioContext && runtimeType == other.runtimeType && android == other.android && iOS == other.iOS;
  }
...
class AudioContextAndroid {
  ...
  @override
  int get hashCode => Object.hash(isSpeakerphoneOn, audioMode, stayAwake, contentType, usageType, audioFocus);
...
class AudioContextIOS {
  // Note when changing the defaults, it should also be changed in native code.
  AudioContextIOS({this.category = AVAudioSessionCategory.playback, this.options = const {}})
    : assert(
      ...
      assert(
        category == AVAudioSessionCategory.playback ||
            category == AVAudioSessionCategory.playAndRecord ||
            category == AVAudioSessionCategory.multiRoute ||
            !options.contains(AVAudioSessionOptions.interruptSpokenAudioAndMixWithOthers),
      ...
      assert(
        category == AVAudioSessionCategory.playAndRecord || !options.contains(AVAudioSessionOptions.allowAirPlay),
        'You can set the option `allowAirPlay` explicitly only if the '
        'audio session category is `playAndRecord`.',
      ),
      assert(
        category == AVAudioSessionCategory.playAndRecord || !options.contains(AVAudioSessionOptions.defaultToSpeaker),
      ...
      assert(
        category == AVAudioSessionCategory.playAndRecord ||
            category == AVAudioSessionCategory.record ||
            category == AVAudioSessionCategory.multiRoute ||
            !options.contains(AVAudioSessionOptions.overrideMutedMicrophoneInterruption),
  ...
  AudioContextIOS copy({AVAudioSessionCategory? category, Set<AVAudioSessionOptions>? options}) {
    return AudioContextIOS(category: category ?? this.category, options: options ?? this.options);
  }

  Map<String, dynamic> toJson() {
    return <String, dynamic>{'category': category.name, 'options': options.map((e) => e.name).toList()};
  }
  ...
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is AudioContextIOS &&
            ...
            const SetEquality<AVAudioSessionOptions>().equals(options, other.options);
  }
