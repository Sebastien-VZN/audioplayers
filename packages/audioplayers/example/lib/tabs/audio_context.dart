...
class AudioContextTabState extends State<AudioContextTab> with AutomaticKeepAliveClientMixin<AudioContextTab> {
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            ElevatedButton.icon(icon: const Icon(Icons.undo), label: const Text('Reset'), onPressed: () => updateConfig(AudioContextConfig())),
          ],
        ),
        ...
        Expanded(
          child: Tabs(
            tabs: [
              TabData(key: 'contextTab-genericFlags', label: 'Generic Flags', content: _genericTab()),
              if (!kIsWeb && defaultTargetPlatform == TargetPlatform.android)
                TabData(key: 'contextTab-android', label: 'Android', content: _androidTab()),
              if (!kIsWeb && defaultTargetPlatform == TargetPlatform.iOS) TabData(key: 'contextTab-ios', label: 'iOS', content: _iosTab()),
            ],
          ),
        ),
      ],
  ...
  Future<void> updateAudioContextAndroid(AudioContextAndroid contextAndroid) async {
    audioContext = audioContext.copy(android: contextAndroid);
    await _applyAudioContext(audioContext);
    setState(() {});
  }

  Future<void> updateAudioContextIOS(AudioContextIOS Function() buildContextIOS) async {
    try {
  ...
  Widget _genericTab() {
    return TabContent(
      children: [
        ...
        Cbx('Respect Silence', value: audioContextConfig.respectSilence, ({value}) => updateConfig(audioContextConfig.copy(respectSilence: value))),
        Cbx('Stay Awake', value: audioContextConfig.stayAwake, ({value}) => updateConfig(audioContextConfig.copy(stayAwake: value))),
      ],
    );
  }
    ...
    return TabContent(
      children: [
        Cbx(
          ...
          ({value}) => updateAudioContextAndroid(audioContext.android.copy(isSpeakerphoneOn: value)),
        ),
        Cbx('stayAwake', value: audioContext.android.stayAwake, ({value}) => updateAudioContextAndroid(audioContext.android.copy(stayAwake: value))),
        LabeledDropDown<AndroidContentType>(
          ...
          onChange: (v) => updateAudioContextAndroid(audioContext.android.copy(contentType: v)),
        ),
        LabeledDropDown<AndroidUsageType>(
          ...
          onChange: (v) => updateAudioContextAndroid(audioContext.android.copy(usageType: v)),
        ),
        LabeledDropDown<AndroidAudioFocus?>(
          ...
          onChange: (v) => updateAudioContextAndroid(audioContext.android.copy(audioFocus: v)),
        ),
        LabeledDropDown<AndroidAudioMode>(
          ...
          onChange: (v) => updateAudioContextAndroid(audioContext.android.copy(audioMode: v)),
        ),
      ],
    );
  }
