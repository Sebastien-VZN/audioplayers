...
class AudioPlayer {
    ...
    _onPlayingStateUpdateSubscription = onPlayingStateUpdate.listen(
      (isPlaying) async {
        ...
        if (state != updatedState) {
          ...
          //
          // On the other hand the action can be triggered by the system
  ...
  Future<void> stop() async {
    ...
    // Update position, once the async platform stop has finished.
    // The position update in 'onPlayingStateUpdateSubscription' sometimes is
    // not sufficient, as seeking to 0 still takes place asynchronously after
    // the player has emitted the pause/stop event.
  ...
  Future<void> _completePrepared(Future<void> Function() setSource) async {
    ...
    if (state == PlayerState.playing) {
      ...
      // The desire might still be to play the source, after it has been set.
      await _stopWithoutDesire();
    }
  ...
  Future<void> _completePlayingStateUpdate(
    ...
    if (desiredState != playerState) {
      ...
      // It is therefore obsolete and handled by another method.
      return;
    }
