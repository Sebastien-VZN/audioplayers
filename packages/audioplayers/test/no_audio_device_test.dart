...
void main() {
  ...
  group('Issue #1979 — No audio device (creation failure)', () {
    test('Player creation with PlatformException does not crash the app', () async {
      ...
      expect(player.creatingCompleter.future, throwsA(isA<PlatformException>().having((e) => e.code, 'code', 'WindowsAudioError')));

      // Le player ne doit pas crasher — il reste dans l'état initial.
      await expectLater(player.creatingCompleter.future, throwsA(isA<PlatformException>()));
    });

    test('play() after creation failure logs error without crashing', () async {
      when(() => mockPlatform.create(any())).thenThrow(PlatformException(code: 'WindowsAudioError', message: 'Failed to create audio player.'));
      ...
      await expectLater(player.play(UrlSource('https://example.com/audio.mp3')), throwsA(isA<Exception>()));
    });
  });
  ...
  group('Issue #1979 — No audio device (playback failure)', () {
    test('resume() with PlatformException does not crash the app', () async {
      ...
      when(
        () => mockPlatform.resume(any()),
      ).thenThrow(PlatformException(code: 'WindowsAudioError', message: 'Playback error', details: 'Échec de la création de mediasink (Code: 4)'));
      ...
      expect(player.resume(), throwsA(isA<PlatformException>().having((e) => e.code, 'code', 'WindowsAudioError')));
    });

    test('play() catches PlatformException on resume and logs via AudioLogger', () async {
      final player = AudioPlayer()..audioCache = mockCache;
      await player.creatingCompleter.future;
    ...
    test('BytesSource playback failure does not crash', () async {
      ...
      final playFuture = player.play(BytesSource(bytes, mimeType: 'audio/mpeg'));
  ...
  group('Issue #1979 — Error stream handling', () {
    test('Platform error on event stream does not crash player', () async {
      ...
      eventController.addError(
        PlatformException(code: 'WindowsAudioError', message: 'Playback error', details: 'Échec de la création de mediasink (Code: 4)'),
      );
    ...
    test('Multiple sequential errors do not crash player', () async {
      ...
      for (var i = 0; i < 5; i++) {
        eventController.addError(PlatformException(code: 'WindowsAudioError', message: 'Playback error attempt $i'));
      }
